...
<section>
    ...
    <div id="hire-container">
        ...
        <div class="hire-cate">
            <ul id="cate-menu">
                <li class="cate-li" id="cateLocation">지역선택</li>
                <li class="cate-li" id="cateJob">직종선택</li>
                <li class="cate-li" id="cateEtc">상세선택</li>
            </ul>
...
<script>
    var cateLocation = $('#cateLocation');
    var cateJob = $('#cateJob');
    var cateEtc = $('#cateEtc');

    $('#search-job').css("display", "none");
    $('#search-etc').css("display", "none");

    cateLocation.click(function () {
        cateLocation.addClass('selected');
        cateJob.removeClass('selected');
        cateEtc.removeClass('selected');
        $('#search-location').css("display", "inline-block");
        $('#search-job').css("display", "none");
        $('#search-etc').css("display", "none");
    });
    cateJob.click(function () {
        cateLocation.removeClass('selected');
        cateJob.addClass('selected');
        cateEtc.removeClass('selected');
        $('#search-location').css("display", "none");
        $('#search-job').css("display", "inline-block");
        $('#search-etc').css("display", "none");
    });
    cateEtc.click(function () {
        cateLocation.removeClass('selected');
        cateJob.removeClass('selected');
        cateEtc.addClass('selected');
        $('#search-location').css("display", "none");
        $('#search-job').css("display", "none");
...
<script>
    ...
    var etc = $('.search_right #etc');
    $.each(etc, function (index) {
        etc.eq(index).hide();
    });
    etc.eq(0).show();

    $('#search-etc .search_left ul li').click(function () {
        switch ($(this).text()) {
            case "연봉":
                $.each(etc, function (index) {
                    etc.eq(index).hide();
...
<script>
    ...
    $('#search-job ul li').click(function () {
        if (!$(this).hasClass('selected')) {
            if ($(this).text().indexOf("전체") > 0) {
                var li = $('#search-job ul li');
                for (var i = 0; i < li.length; i++) {
                    li[i].removeAttribute("class");
                }
            ...
            $(this).addClass('selected');
            searchOption.append("<button onclick='fn_selectCancleJob(this)'>" + $(this).text().trim() + "　<i class='fas fa-backspace'></i></button>");
        } else {
            $(this).removeClass('selected');
            var btn = $('#select_left button');
            for (var i = 0; i < btn.length; i++) {
    ...
    $('#etc ul li').click(function () {
        if (!$(this).hasClass('selected')) {
            ...
            $(this).addClass('selected');
            searchOption.append("<button onclick='fn_selectCancleEtc(this)'>" + $(this).text().trim() + "　<i class='fas fa-backspace'></i></button>");
        } else {
            $(this).removeClass('selected');
            var btn = $('#select_left button');
            for (var i = 0; i < btn.length; i++) {
    ...
    function fn_paging(cPage) {
        window.location.href = "${path}/hireNotiAll.do?cPage=" + cPage;
    }

    function fn_ajaxPaging(cPage) {
        var result = $('#hire-list');
        var category_temp = $('#select_left button');
        var category = "";

        for (var i = 0; i < category_temp.length; i++) {
            category += category_temp[i].innerText.trim() + ",";
        }


        $.ajax({
            url: '${path}/hireNotiAllAjax.do',
    ...
    function fn_search() {
        ...
        if (category_temp.length != 0) {
            for (var i = 0; i < category_temp.length; i++) {
                if (category_temp[i].innerText.trim().indexOf("전체") == 0) {
                    category += "";
                } else {
                    category += category_temp[i].innerText.trim() + ",";
                }
